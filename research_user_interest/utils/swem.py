...
class SWEM:
    ...
    def __init__(self, w2v, tokenizer, oov_initialize_range=(-0.01, 0.01)):
        ...
        if self.oov_initialize_range[0] > self.oov_initialize_range[1]:
            raise ValueError(
                "Specify valid initialize range: "
    ...
    def hierarchical_pooling(self, text, n):
        ...
        if n > text_len:
            raise ValueError(
                f"window size must be less than text length "
                f"window_size:{n} text_length:{text_len}"
            )
        window_average_pooling_vec = [
            np.mean(word_embeddings[i : i + n], axis=0) for i in range(text_len - n + 1)
        ]
